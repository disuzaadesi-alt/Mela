...
// body: { items: [{productId, qty}], address: {...}, paymentMethod: 'upi'|'card'|'cod' }
...
// GET /api/orders/all - admin: every order from every customer
router.get("/all", requireAuth, requireAdmin, async (req, res) => {
  const orders = await prisma.order.findMany({
    include: {
      items: { include: { product: { select: { imageSeed: true, category: true } } } },
...
// PATCH /api/orders/:id/stage - admin: move an order to the next fulfillment stage
